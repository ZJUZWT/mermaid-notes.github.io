...
const CONFIG = {
    ...
    nodeDetails: {
        ...
        {
            variables: [
                { name: 'Ability System Component (ASC)', type: '协处理器 / 专用芯片 (Co-processor / ASIC)', desc: '安装在Actor上的核心处理单元。它独立于主CPU（Actor的常规逻辑），专门负责接收、调度和执行所有与游戏能力相关的指令和后台服务。'},
                { name: 'AttributeSet & Attributes', type: '内存中的特定数据段与变量 (Specific Data Segment & Variables in Memory)', desc: '由ASC直接管理的一块专用、受保护的内存区域。每个Attribute都是其中一个地址明确的变量，其读写必须通过GAS内核的授权（即通过GE），而不能直接操作。'},
                { name: 'Gameplay Effect (GE)', type: '向操作系统内核提交的“工作请求单” (A "Work Request" Submitted to the OS Kernel)', desc: '这是对GE最精准的描述。它不是一条简单的指令，而是向“GAS操作系统”提交的一份请求，要求其改变某个目标的状态。这份请求单根据其类型，会被内核以不同方式处理'},
                { name: 'Instant GE (即时GE)', desc: '被处理为一条简单的CPU算术/逻辑指令，立即执行并完成。'},
                { name: 'Duration / Periodic GE (持续/周期性GE)', desc: '被处理为注册一个受时间管理的后台服务/进程。操作系统会为其分配资源，并根据设定的时间（持续/周期）由系统时钟进行调度，直到服务生命周期结束。'},
                { name: 'Gameplay Ability (GA)', type: '可执行函数 / 脚本 (Executable Function / Script)', desc: '一个封装了完整交互逻辑的高级函数。它本身不直接修改数据，而是通过编写代码逻辑，在合适的时机向ASC（协处理器）提交GE（工作请求单），或发起其他异步任务。'},
                { name: 'GameplayTag', type:'系统级事件与状态信道 (System-Level Event and State Channel)', desc: '一个高效的、层级化的通信总线。它不存储数据，而是用于广播和监听系统状态（如State.Stunned）。各个模块（GA, GE, 动画蓝图等）可以通过监听特定信道来触发反应，或通过检查信道状态来决定自身行为，是系统解耦的神经中枢。'},
                { name: 'AbilityTask', type:'协程 / 异步系统调用 (Coroutine / Asynchronous System Call)', desc: '在GA这个“函数”内部，向“GAS操作系统”发起的一个非阻塞的异步请求（如“等待用户输入”、“等待网络回调”）。当前函数执行流会在此挂起，待系统完成任务后，通过回调恢复执行，从而避免了主线程的阻塞。'},
                { name: 'GameplayCue (GC)', type:'系统事件总线上的“即发即弃”消息 (Fire-and-Forget Message on an Event Bus)', desc: '当核心逻辑发生时，向一个公共的、非关键的事件总线广播的一条消息。它用于通知外部系统（如特效、声音）进行响应。这条消息的设计目的是为了表现与逻辑的彻底分离，即使无人监听，也不会影响核心逻辑的执行。'},
                { name: 'ExecutionCalculation', type:'可插拔的自定义数学协处理器 / 函数指针 (Pluggable Math Co-processor / Function Pointer)', desc: '一个指向C++代码的函数指针，专门用于处理复杂的数值计算。当GE这份“工作请求单”需要一个动态计算的结果时，ASC（协处理器）会暂停处理，将输入参数打包，调用这个外挂的“数学处理器”来获得结果，然后再继续执行。'},
            ],
            methods: [
                {
                    name: '理解',
                    desc: '按照上面的逻辑，就可以知道很多事实，这也是GAS里面非常容易被混淆的。<ul>\
                    <li>批处理脚本里面，可以处理非\"工作请求\"逻辑，也就是GA的Activate完全可以脱离GE。</li>\
                    <li>批处理脚本派生的\"工作请求\"的生命周期，与批处理脚本无关。</li>\
                    <li>信道可以被所有方法填充，任何部件都可以从信道中Query，并且信道具有广播能力。</li>\
                    <li>内存既可以被\"工作请求\"修改，也可以被外部强行修改，但是都需要过处理器的接口。</li></ul>'
                }
            ]
        },
        'GT_Tutorial':
